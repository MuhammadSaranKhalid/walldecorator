...
export function ObsidianHeroSection({
  title = 'Elevate Your',
  subtitle = 'Space',
  titleLine3,
  eyebrow = 'Laser-Cut Wall Art Decorator Pieces',
  description = 'Precision-crafted laser-cut designs that turn your walls into a gallery. Custom sizes and premium materials for every vision.',
  primaryCTA = { text: 'Explore Collection', href: '/products' },
  secondaryCTA = { text: 'Custom Order', href: '/custom-order' },
  categories = ['Anime', 'Islamic', 'Nature', 'Geometric', 'Abstract'],
}: HeroSectionProps) {
  return (
    <section className="pt-20 min-h-[96vh] grid grid-cols-1 lg:grid-cols-2 items-center overflow-hidden relative z-[1]">
      {/* Left Content */}
      <div className="px-6 sm:px-10 lg:px-14 py-20 lg:py-20">
        {/* Eyebrow */}
        ...
        {/* Title */}
        <h1 className="font-[family-name:var(--font-cormorant)] text-[clamp(52px,6.5vw,92px)] font-light leading-[0.95] mb-8">
          {title}
          <br />
          <em className="italic text-[var(--obsidian-gold)]">{subtitle}</em>
          {titleLine3 && (
            <>
              <br />
              {titleLine3}
        ...
        {/* Subtitle */}
        <p className="text-[13px] leading-[1.9] text-[var(--obsidian-text-muted)] max-w-[400px] mb-10">
          {description}
        </p>
        ...
        <div className="flex flex-wrap gap-2 mb-11">
          {categories.map((category) => (
            <button
              key={category}
              className="text-[9px] tracking-[0.125em] uppercase px-3.5 py-1.5 border border-[var(--obsidian-border)] text-[var(--obsidian-text-dim)] cursor-pointer transition-all duration-200 hover:border-[var(--obsidian-gold)] hover:text-[var(--obsidian-gold)]"
            >
              {category}
            </button>
          ))}
      ...
      <div className="lg:hidden h-[72vw] min-h-[280px] bg-[var(--obsidian-surface)] relative overflow-hidden flex items-center justify-center">
        <div className="relative z-[2] animate-heroFloat">
